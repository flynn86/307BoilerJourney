...
func _process(delta):
	...
	if Input.is_action_just_pressed("chat"):
		print("chatting")
		$dialogue_character_2_winter.start()
		is_roaming = false
		is_chatting = true
		$AnimatedSprite2D.play("Idle")
...
func _on_chat_detection_area_body_exited(body):
	if body.has_method("Player"):
		...
		get_node("dialogue_character_2_winter/NinePatchRect").visible = false
		$dialogue_character_2_winter.d_active = false
		_on_dialogue_character_2_winter_dialogue_finished()


func _on_dialogue_character_2_winter_dialogue_finished():
	is_chatting = false
	is_roaming = true
