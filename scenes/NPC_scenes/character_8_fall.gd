...
func _on_chat_detection_area_body_entered(body):
	...
	if body.name == "CharacterBody2D":
		...
		#print("chatting")
		$dialogue_character_8_fall.start()
		is_roaming = false
		is_chatting = true
		$AnimatedSprite2D.play("Idle")
...
func _on_chat_detection_area_body_exited(body):
	if body.name == "CharacterBody2D":
		...
		get_node("dialogue_character_8_fall/NinePatchRect").visible = false
		$dialogue_character_8_fall.d_active = false
		_on_dialogue_character_8_fall_dialogue_finished()


func _on_dialogue_character_8_fall_dialogue_finished():
	is_chatting = false
	is_roaming = true
